...
const MusicBox = ({
  ...
  return onPress ? (
    <TouchableOpacity style={styles.musicBox} onPress={onPress}>
      {content}
    </TouchableOpacity>
  ) : (
...
const PassportDetail = ({
  ...
  const [editCategory, setEditCategory] = useState(
    CATEGORY_TO_KOREAN[item.category] ?? "선택",
  );
  ...
  const [isCover, setIsCover] = useState(
    !!(
      item._districtThumbnailUrl &&
      (item.imageUrls?.[0] ?? item.thumbnailUrl) === item._districtThumbnailUrl
    ),
  );
  ...
  const [editDistrict, setEditDistrict] = useState(
    item.district ?? item.districtDisplayName ?? "",
  );
  const [editDistrictCategory, setEditDistrictCategory] = useState(
  ...
  const districtCount =
    (item as any)._districtPassportCount ??
    districts?.find((d) => d.displayName === editDistrict)?.passportCount ??
    0;

  const displayMusic: MusicDisplayItem =
    editMusic ??
    (item.musicTitle
      ? {
  ...
  useEffect(() => {
    if (item.musicTitle && item.musicArtist) {
      fetch(
        `https://itunes.apple.com/search?term=${encodeURIComponent(item.musicTitle + " " + item.musicArtist)}&media=music&limit=1`,
      )
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleDelete = async () => {
    Alert.alert("여권 삭제", "해당 여권을 삭제하시겠습니까?", [
  ...
  const openImagePicker = async () => {
    ...
    if (!result.canceled) {
      ...
      setEditImageUris(
        await compressImagesForUpload(result.assets.map((a) => a.uri)),
      );
    }
  };
  ...
  const handleSave = async () => {
    try {
      const matchedDistrict = districts?.find(
        (d) => d.displayName === editDistrict,
      );
      const originalDistrict = item.district ?? item.districtDisplayName ?? "";
      const isNewDistrict =
        editDistrict !== originalDistrict && !matchedDistrict;
      ...
      const passportData: CreatePassportRequest = {
        ...
        districtCategory:
          matchedDistrict?.districtCategory ??
          DISTRICT_CATEGORY_MAP[editDistrict] ??
          editDistrictCategory,
      ...
      if (isNewDistrict) {
        ...
        const isPlaceChanged = !!item.address && editAddress !== item.address;
      ...
      Alert.alert(
        "저장 실패",
        JSON.stringify(errData ?? e.message ?? "알 수 없는 오류"),
      );
    }
  };
    ...
    try {
      ...
      if (coverId == null) {
        const districtsRes = await getMyPassportDistricts(
          isTeamMode && teamId ? teamId : undefined,
        );
        const allDistricts: any[] = districtsRes.data?.data ?? [];
        const match = allDistricts.find(
          (d: any) =>
            d.districtCategory === editDistrictCategory ||
            d.displayName === editDistrict,
        );
      ...
      >
        ...
        <View style={styles.passportCard}>
          ...
          <View
            style={[
              styles.dividerRow,
              { position: "absolute", top: scaleH(300), left: 0, right: 0 },
            ]}
          >
              ...
              <FlatList
                ...
                renderItem={({ item: option }) => (
                  <TouchableOpacity
                    style={[
                      styles.modalItem,
                      option === editCategory && styles.modalItemSelected,
                    ]}
                    onPress={() => {
                  ...
                  >
                    <Text
                      style={[
                        ...
                        option === editCategory && styles.modalItemTextSelected,
                      ]}
                    >
                      {option}
                    </Text>
        ...
        <KakaoPlaceSearch
          ...
          onSelect={(place) => {
            ...
            const guPart = addressStr
              .split(" ")
              .find((token) => token.endsWith("구"));
