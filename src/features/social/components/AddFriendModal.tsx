...
export default function AddFriendModal({
  ...
  const [recommendedFriends, setRecommendedFriends] = useState<
    RecommendedFriend[]
  >([]);
  const [requestedFriendKeys, setRequestedFriendKeys] = useState<Set<string>>(
    new Set(),
  );
  ...
  const filterRecommendedFriendsByNickname = (
    ...
    return applyRequestedStatus(
      sourceFriends.filter((friend) =>
        friend.nickname?.toLowerCase().includes(trimmedText),
      ),
      requestedFriendKeys,
    );
  };
  ...
  const fetchRecommendedFriends = async (
    ...
    try {
      ...
      setFriends(
        filterRecommendedFriendsByNickname(keyword, friendsWithRequestedStatus),
      );
    } catch (error) {
  ...
  const handleSearchFriends = async () => {
    ...
    try {
      ...
      const nicknameMatches = recommendedFriends.filter((friend) =>
        friend.nickname?.toLowerCase().includes(trimmedKeyword.toLowerCase()),
      );
