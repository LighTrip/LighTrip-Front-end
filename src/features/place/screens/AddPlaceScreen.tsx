...
const AddPlaceScreen = ({ isSubscribed = true }: Props) => {

    const router = useRouter()

    const [showEdit, setShowEdit] = useState(false)
    const [photos, setPhotos] = useState<string[]>([])
    const selectionLimit = isSubscribed ? 5 : 1
    ...
    const openAlbum = async () => {
        ...
        const result = await ImagePicker.launchImageLibraryAsync({
            mediaTypes: ['images'],
            allowsEditing: false,
            allowsMultipleSelection: isSubscribed,
            selectionLimit,
            quality: 1,
            exif: true,
        })
            ...
            >
                <View style={styles.photoContainer}>
                    ...
                    {photos.length > 1 ? (
                        ...
                        <TouchableOpacity style={styles.albumButtonEmpty} onPress={openAlbum}>
