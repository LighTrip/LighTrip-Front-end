import { getClusterDetail, getMyLights, ClusterDetailItem, LightItem } from "@/src/api/lights.api";
...
function clusterSingleMarkers(items: LightItem[], bbox: BBox | null): FrontCluster[] {
...
const CATEGORY_ICONS: Record<string, React.FC<{ width?: number; height?: number; fill?: string }>> = {
  CAFE:       CafeIcon,
...
/** 서버 클러스터 마커 — 56×56 */
function ServerClusterMarker({ count, category }: { count: number; category: string }) {
  const { main, light } = CATEGORY_COLORS[category] ?? CATEGORY_COLORS.ETC;
  return (
    <Svg width={56} height={56} viewBox="0 0 56 56">
      <GlowMarker cx={28} cy={28} r={28} rMid={20} rInner={15} main={main} light={light}>
        <SvgText x={28} y={33} textAnchor="middle" fill="white" fontSize={13} fontWeight="bold"
...
/** 프론트 클러스터 마커 — 64×64 */
function FrontClusterMarker({ count, category }: { count: number; category: string }) {
  const { main, light } = CATEGORY_COLORS[category] ?? CATEGORY_COLORS.ETC;
  return (
    <Svg width={64} height={64} viewBox="0 0 64 64">
      <GlowMarker cx={32} cy={32} r={32} rMid={23} rInner={17} main={main} light={light}>
        <SvgText x={32} y={38} textAnchor="middle" fill="white" fontSize={15} fontWeight="bold"
          clipPath="url(#gm_clip)">
          {String(count)}
        </SvgText>
      </GlowMarker>
          ...
          {items.map((item, idx) => (
            ...
            >
              <View style={styles.sheetItemIcon}>
                {(() => { const Icon = CATEGORY_ICONS[item.category] ?? EtcIcon; return <Icon width={24} height={24} fill={(CATEGORY_COLORS[item.category] ?? CATEGORY_COLORS.ETC).main} />; })()}
              </View>
...
export default function MapScreen() {
  ...
  const [activeClusterItems, setActiveClusterItems] = useState<ClusterDetailItem[] | null>(null);
  ...
  const handleServerClusterTap = async (item: LightItem) => {
    ...
    try {
      ...
      if (content.length === 1) {
        handleSinglePinTap({ ...content[0], latitude: lat, longitude: lng, isCluster: false } as any);
      } else if (content.length > 1) {
        setActiveClusterItems(content);
      } else {
        mapRef.current?.animateCameraTo({ latitude: lat, longitude: lng, zoom: 16, duration: 400 });
      }
    } catch {
      mapRef.current?.animateCameraTo({ latitude: lat, longitude: lng, zoom: 16, duration: 400 });
    }
  ...
  const handleFrontClusterTap = async (cluster: FrontCluster) => {
    ...
    try {
      ...
      setActiveClusterItems(cluster.items.map((it) => ({
  ...
  const handleSheetItemSelect = (item: ClusterDetailItem) => {
    ...
    handleSinglePinTap({ ...item, latitude: 0, longitude: 0, isCluster: false } as any);
  };
      ...
      >
        ...
        {lights
          ...
          .map((item, idx) => (
            ...
            >
              <ServerClusterMarker count={item.count ?? 0} category={item.category} />
            </NaverMapMarkerOverlay>
          ))}
        ...
        {frontClusters.map((cluster) =>
          cluster.items.length === 1 ? (
            ...
            >
              <PassportMarker spaceName={cluster.items[0].spaceName} category={cluster.items[0].category} />
            </NaverMapMarkerOverlay>
          ) : (
            <NaverMapMarkerOverlay
